...
class Distributors extends Admin_Controller
{	
	/**
	 * Constructor
	 * 
	...
	public function index()
	{
		if ($this->input->post(null, true)) {
			...
			$this->datatables
			->select('admin.admin_id, admin.name, admin.email, admin.username, admin.active, concat(warehouse.code," - ",warehouse.name) as distributor')
			->where('is_distributor', '1')
			->from('admin')
			->join('warehouse', 'warehouse.warehouse_id = admin.warehouse_id', 'left');
			...
			->breadcrumb(lang('heading_title2'))
			->view('admin/distributor');
		}
	}
	...
	public function create()
	{
		...
		$this->load->vars(array('heading_title' => lang('heading_create2')));
	...
	public function edit()
	{
		...
		$this->load->vars(array('heading_title' => lang('heading_edit2')));
	...
	{
		$data['action'] = admin_url('distributors/validate');
		$data['admin_id'] = null;
		$data['name'] = '';
		$data['username'] = '';
		$data['email'] = '';
		...
		if ($admin = $this->admin_model->get($admin_id)) {
			...
			$data['warehouse_id'] = (int)$admin['warehouse_id'];
		}

		foreach ($this->warehouse_model->get_all_warehouse() as $warehouse) {
			if($warehouse['warehouse_id'] != 2) {
				$data['warehouse'][$warehouse['warehouse_id']] = $warehouse['code'].' ('.$warehouse['name'].')';
			}
		}
		
		$this->output->set_output(json_encode(array(
			'content' => $this->load->layout(false)->view('admin/distributor_form', $data, true)
		)));
	}
	...
	public function validate()
	{
		...
		->set_rules('warehouse_id', 'Gudang / Distributor', 'trim|required')
		...
		if ($this->form_validation->run() == false) {
			...
			$data['is_distributor'] = $this->input->post('is_distributor');
