		...
		<div class="card">
			<div class="card-body">
				<div class="form-action">
					<a onclick="getForm(null, '/admins/create');" class="btn btn-primary"><i class="fa fa-plus-circle"></i> <?=lang('button_insert')?></a>
					<a onclick="delRecord();" class="btn btn-default"><i class="fa fa-check-square-o"></i> <?=lang('button_remove')?></a>
				</div>
				<div id="message"></div>
				<div class="table-responsive">
					<?=form_open(admin_url('admins/delete'), 'id="bulk-action"')?>
	...
	$(document).ready(function() {
		var table = $('#datatable').DataTable({
			...
			"createdRow": function (row, data, index) {
				...
				html += '<a onclick="getForm('+data.admin_id+', \'/admins/edit\');" class="btn btn-default btn-xs"><i class="fa fa-cog"></i> <?=lang('button_edit')?></a>';
				html += '</div>';
				
				$('td', row).eq(5).addClass('text-right').html(html);
				
				if (data.active === '1') {
					$('td', row).eq(4).html('<span class="label label-success"><?=lang('text_active')?></span>');
				} else {
					$('td', row).eq(4).html('<span class="label label-default"><?=lang('text_inactive')?></span>');
				}
			},
			"order": [[0, 'asc']]
		});
