...
<script type="text/javascript">
	$(document).ready(function() {
	    var table = $('#datatable').DataTable({
			...
			"createdRow": function (row, data, index) {
				...
				if (data.date_paid == '') {
					html += '<a onclick="approve('+data.user_transaction_id+');" class="btn btn-primary btn-xs"><i class="fa fa-check"></i> Approve</a>';
				}
				...
				if (data.date_paid == '') {
					$('td', row).eq(4).html('<span class="label label-warning">Tunggu Transfer</span>');
				}
	...
	function delRecord(user_transaction_id) {
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url : '<?=admin_url('user_topup/delete')?>',
					type : 'post',
	...
	function approve(user_transaction_id) {
		swal({
			...
			text: "Proses approval tidak dapat dianulir!",
			type: "warning",
			showCancelButton: true,
			confirmButtonText: "Ya",
			cancelButtonText: "Tidak",
			...
			if (isConfirm) {
				$.ajax({
					url : '<?=admin_url('user_topup/approve')?>',
					type : 'post',
					data: 'user_transaction_id='+user_transaction_id,
					dataType: 'json',
					success: function(json) {
