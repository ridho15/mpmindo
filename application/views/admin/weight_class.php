	...
	<div class="card">
		<div class="card-body">
			<div class="form-action">
				<a onclick="getForm(null, '/weight_class/create');" class="btn btn-primary"><i class="fa fa-plus-circle"></i> <?=lang('button_insert')?></a>
				<a onclick="delRecord();" class="btn btn-default"><i class="fa fa-check-square-o"></i> <?=lang('button_remove')?></a>
			</div>
			<div class="table-responsive">
				<?=form_open(admin_url('weight_class/delete'), 'id="bulk-action"')?>
...
<script type="text/javascript">
	$(document).ready(function() {
		var table = $('#datatable').DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": "<?=admin_url('weight_class')?>",
				"type": "POST",
			},
			"columns": [
				{"orderable": false, "searchable" : false, "data": "weight_class_id"},
				{"data": "title"},
				{"data": "unit"},
				{"data": "value"},
				{"orderable": false, "searchable" : false, "data": "weight_class_id"},
			...
			"createdRow": function (row, data, index) {
				$('td', row).eq(0).html('<input type="checkbox" class="checkbox" name="weight_class_id[]" value="'+data.weight_class_id+'"/>');
				if (data.weight_class_id === "<?=$this->config->item('weight_class_id')?>") {
				...
				html += '<a onclick="getForm('+data.weight_class_id+', \'/weight_class/edit\');" class="btn btn-default btn-xs"><i class="fa fa-pencil"></i> <?=lang('button_edit')?></a>';
				html += '</div>';
	...
	function delRecord() {
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url : '<?=admin_url('weight_class/delete')?>',
					type : 'post',
	...
	function getForm(weight_class_id, path) {
		$.ajax({
