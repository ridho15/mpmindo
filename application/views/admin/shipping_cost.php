	...
	<div class="card">
		<div class="card-body">
			<div class="form-action">
				<a onclick="getForm(null, '/shipping_cost/create');" class="btn btn-primary"><i class="fa fa-plus-circle"></i> <?=lang('button_insert')?></a>
				<a onclick="delRecord();" class="btn btn-default"><i class="fa fa-check-square-o"></i> <?=lang('button_remove')?></a>
			</div>
			<div class="table-responsive">
				<?=form_open(admin_url('shipping_cost/delete'), 'id="bulk-action"')?>
...
<script type="text/javascript">
	$(document).ready(function() {
		var table = $('#datatable').DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": "<?=admin_url('shipping_cost')?>",
				"type": "POST",
			},
			"columns": [
				{"orderable": false, "searchable" : false, "data": "shipping_cost_id"},
				{"data": "province"},
				{"data": "city"},
				{"data": "subdistrict"},
				{"data": "cost"},
			...
			"createdRow": function (row, data, index) {
				$('td', row).eq(0).html('<input type="checkbox" class="checkbox" name="shipping_cost_id[]" value="'+data.shipping_cost_id+'"/>');
				
				html  = '<div class="btn-group btn-group-xs">';
				html += '<a onclick="getForm('+data.shipping_cost_id+', \'/shipping_cost/edit\');" class="btn btn-default btn-xs"><i class="fa fa-pencil"></i> <?=lang('button_edit')?></a>';
				html += '</div>';
	...
	function delRecord() {
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url : '<?=admin_url('shipping_cost/delete')?>',
					type : 'post',
	...
	function getForm(shipping_cost_id, path) {
		$.ajax({
			url : $('base').attr('href')+path,
			data: 'shipping_cost_id='+shipping_cost_id,
			dataType: 'json',
			success: function(json) {
				if (json['error']) {
					$.notify({
